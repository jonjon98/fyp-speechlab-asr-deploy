...
if [ "$1" = 'project' ]; then
    if [ "$2" = 'up' ]; then
        ...
        kubectl apply -f default/decoding-sdk-server-service.yaml
        kubectl apply -f default/decoding-sdk-server-deployment.yaml
        kubectl apply -f default/decoding-sdk-worker-storageclass.yaml
        kubectl apply -f default/decoding-sdk-worker-persistentvolume.yaml
        kubectl apply -f default/decoding-sdk-worker-claim0-persistentvolumeclaim.yaml
        kubectl apply -f default/decoding-sdk-worker-deployment.yaml
    
    elif  [ "$2" = 'down' ]; then
        echo "Removing ASR project from K8s cluster"

        kubectl delete -f default/decoding-sdk-server-deployment.yaml
        kubectl delete -f default/decoding-sdk-server-service.yaml
        kubectl delete -f default/decoding-sdk-worker-deployment.yaml
        kubectl delete -f default/decoding-sdk-worker-claim0-persistentvolumeclaim.yaml
        kubectl delete -f default/decoding-sdk-worker-persistentvolume.yaml
        kubectl delete -f default/decoding-sdk-worker-storageclass.yaml
    ...
    echo "Usage ./build.sh [project/ingress/help] [up/down]"
    exit
